...
CREATE POLICY "agenda_events_all_own" ON public.agenda_events FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());-- SoloForte — Schema Supabase completo (Fase 1 + Fase 2)
-- Executar no SQL Editor do Supabase
...
DROP POLICY IF EXISTS "agenda_events_all_own" ON public.agenda_events;
CREATE POLICY "agenda_events_all_own" ON public.agenda_events FOR ALL TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());-- Arquivo para executar no Dashboard do Supabase (SQL Editor)
