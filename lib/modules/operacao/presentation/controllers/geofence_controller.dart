...
import '../../domain/entities/geofence_state.dart';
...
class GeofenceController {
  ...
  Future<void> _checkGeofence() async {
    ...
    // Get active event from agenda (em andamento)
    final activeEvent = agendaState.events
        .where(
          (e) =>
              e.visitSessionId != null && e.status == EventStatus.emAndamento,
        )
        .firstOrNull;

    if (activeEvent != null && activeEvent.talhaoId != null) {
      // --- Scenario: Active Session -> Check Exit ---
      // Get the field (talhao) for the active event
      final activeField = fieldsAsync.value!.cast<Talhao?>().firstWhere(
        (f) => f?.id == activeEvent.talhaoId,
        orElse: () => null,
      );
  ...
  Future<void> _checkDuration() async {
    ...
    if (activeEvent != null) {
      final duration = DateTime.now().difference(
        activeEvent.dataInicioPlanejada,
      );
