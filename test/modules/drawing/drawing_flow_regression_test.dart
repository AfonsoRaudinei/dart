...
void main() {
  group('FIX-DRAW-FLOW-02 — Regressão: Trocas Rápidas', () {
    ...
    test(
      '⚡ Trocar ferramenta 10x seguidas não deve lançar erro',
      () {
        expect(
          () {
            for (var i = 0; i < 10; i++) {
    ...
    test(
      '⚡ Alternar entre armed e idle rapidamente',
      () {
        expect(
          () {
            for (var i = 0; i < 20; i++) {
    ...
    test(
      '⚡ Trocar ferramenta durante drawing não deve corromper estado',
      () {
        controller.selectTool('polygon');
        ...
        // Trocar ferramenta no meio do desenho
        controller.selectTool('rectangle');

        // Deve resetar para armed com nova ferramenta
        expect(controller.currentState, equals(DrawingState.armed));
    ...
    test(
      '⚡ Múltiplas trocas com pontos não deve vazar memória',
      () {
        for (var i = 0; i < 50; i++) {
          controller.selectTool('polygon');
  ...
  group('FIX-DRAW-FLOW-02 — Regressão: Cancelamentos', () {
    ...
    test(
      '❌ Cancelar sem ter iniciado não deve quebrar',
      () {
        expect(
    ...
    test(
      '❌ Cancelar múltiplas vezes não deve lançar erro',
      () {
        controller.selectTool('polygon');
        controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
    ...
    test(
      '❌ Cancelar durante armed deve voltar para idle',
      () {
        controller.selectTool('polygon');
        expect(controller.currentState, equals(DrawingState.armed));
    ...
    test(
      '❌ Cancelar e rearmar 100x não deve falhar',
      () {
        expect(
          () {
            for (var i = 0; i < 100; i++) {
  ...
  group('FIX-DRAW-FLOW-02 — Regressão: Tap Behaviors', () {
    ...
    test(
      '🖱️ appendDrawingPoint sem selectTool não deve mudar estado',
      () {
        controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
    ...
    test(
      '🖱️ appendDrawingPoint em idle não deve fazer nada',
      () {
        expect(controller.currentState, equals(DrawingState.idle));
    ...
    test(
      '🖱️ Adicionar 1000 pontos não deve travar',
      () {
        controller.selectTool('polygon');
    ...
    test(
      '🖱️ Adicionar pontos idênticos não deve quebrar',
      () {
        controller.selectTool('polygon');
  ...
  group('FIX-DRAW-FLOW-02 — Regressão: Estado Inconsistente', () {
    ...
    test(
      '🔐 selectTool com valor inválido deve resultar em idle',
      () {
    ...
    test(
      '🔐 selectTool com string vazia deve resultar em idle',
      () {
    ...
    test(
      '🔐 Alternar entre tools válidas e inválidas não deve quebrar',
      () {
        expect(
          () {
            controller.selectTool('polygon');
    ...
    test(
      '🔐 Estado armed com pontos zerados deve ser consistente',
      () {
        controller.selectTool('polygon');
        expect(controller.currentState, equals(DrawingState.armed));
        expect(controller.liveGeometry, isNull);
  ...
  group('FIX-DRAW-FLOW-02 — Regressão: InstructionText Consistency', () {
    ...
    test(
      '📝 instructionText nunca deve retornar vazio',
      () {
    ...
    test(
      '📝 instructionText deve mudar com transições de estado',
      () {
    ...
    test(
      '📝 instructionText deve ser específico para estado armed',
      () {
        controller.selectTool('polygon');
  ...
  group('FIX-DRAW-FLOW-02 — Regressão: Lifecycle', () {
    test(
      '♻️ Criar e descartar controller não deve lançar erro',
      () {
        expect(
          () {
    ...
    test(
      '♻️ Usar controller após dispose não deve travar (silent fail)',
      () {
        final controller = DrawingController(repository: MockDrawingRepository());
        controller.dispose();
    ...
    test(
      '♻️ Múltiplos dispose não devem lançar erro',
      () {
        final controller = DrawingController(repository: MockDrawingRepository());
  ...
  group('FIX-DRAW-FLOW-02 — Regressão: Concurrency Simulation', () {
    ...
    test(
      '⚡ Simulação: usuário indeciso trocando ferramentas',
      () {
        expect(
          () {
            controller.selectTool('polygon');
    ...
    test(
      '⚡ Simulação: cancelar e rearmar imediatamente',
      () {
        for (var i = 0; i < 50; i++) {
          controller.selectTool('polygon');
          controller.appendDrawingPoint(LatLng(-15.78 + i * 0.001, -47.92));
    ...
    test(
      '⚡ Simulação: desenhar, cancelar, redesenhar',
      () {
        // Primeiro desenho
        controller.selectTool('polygon');
        controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
  ...
  group('FIX-DRAW-FLOW-02 — Regressão: Geometria Validity', () {
    ...
    test(
      '📐 liveGeometry deve ser null em idle',
      () {
        expect(controller.liveGeometry, isNull);
      },
    );
    ...
    test(
      '📐 liveGeometry deve ser null em armed',
      () {
        controller.selectTool('polygon');
        expect(controller.liveGeometry, isNull);
      },
    ...
    test(
      '📐 liveGeometry deve ter pontos após appendDrawingPoint',
      () {
        controller.selectTool('polygon');
        controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
        controller.appendDrawingPoint(const LatLng(-15.7802, -47.9293));
    ...
    test(
      '📐 liveAreaHa deve ser 0 sem geometria',
      () {
        expect(controller.liveAreaHa, equals(0.0));
    ...
    test(
      '📐 liveAreaHa deve ser > 0 com polígono válido',
      () {
        controller.selectTool('polygon');
        controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
        controller.appendDrawingPoint(const LatLng(-15.7802, -47.9293));
