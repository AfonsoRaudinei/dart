...
void main() {
  group('FIX-DRAW-FLOW-02 — State Machine Flow', () {
    ...
    test('✅ selectTool(polygon) deve transicionar para armed', () {
      // Act
    ...
    test('✅ appendDrawingPoint deve transicionar de armed para drawing', () {
      // Arrange
      controller.selectTool('polygon');
      expect(controller.currentState, equals(DrawingState.armed));
    ...
    test('✅ Múltiplos pontos devem permanecer em drawing', () {
      // Arrange
      controller.selectTool('polygon');
      controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
    ...
    test('✅ liveGeometry deve conter pontos após appendDrawingPoint', () {
      // Arrange
      controller.selectTool('polygon');
    ...
    test('✅ liveAreaHa deve retornar > 0 após 3+ pontos', () {
      // Arrange
      controller.selectTool('polygon');
    ...
    test('🔁 Trocar ferramenta durante drawing deve ser bloqueado', () {
      ...
      // Act — tentar trocar para outra ferramenta durante drawing
      controller.selectTool('rectangle');

      // Assert — deve permanecer em drawing com ferramenta polygon
      expect(controller.currentState, equals(DrawingState.drawing));
      expect(controller.currentTool, equals(DrawingTool.polygon));
      expect(controller.errorMessage, isNotNull); // Deve ter mensagem de erro
    });

    test('🔁 Trocar ferramenta rapidamente não deve lançar erro', () {
    ...
    test('❌ selectTool(none) deve voltar para idle', () {
      // Arrange
      controller.selectTool('polygon');
      expect(controller.currentState, equals(DrawingState.armed));
    ...
    test('❌ cancelOperation deve voltar para idle', () {
      // Arrange
      controller.selectTool('polygon');
      controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
      expect(controller.currentState, equals(DrawingState.drawing));
    ...
    test('🔄 Regressão: Rearmar após cancelar deve funcionar', () {
      // Arrange
      controller.selectTool('polygon');
      controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
      controller.cancelOperation();
    ...
    test('🔄 Regressão: Múltiplos cancela e rearma não devem lançar erro', () {
      expect(() {
        for (var i = 0; i < 5; i++) {
          controller.selectTool('polygon');
  ...
  group('FIX-DRAW-FLOW-02 — InstructionText Validation', () {
    ...
    test('📝 instructionText deve refletir estado idle', () {
      expect(
    ...
    test('📝 instructionText deve refletir estado armed', () {
      controller.selectTool('polygon');
      expect(
        controller.instructionText,
    ...
    test('📝 instructionText deve mudar após primeiro ponto', () {
      controller.selectTool('polygon');
      controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
    ...
    test('📝 instructionText deve guiar usuário com poucos pontos', () {
      controller.selectTool('polygon');
      controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
      controller.appendDrawingPoint(const LatLng(-15.7802, -47.9293));
    ...
    test('📝 instructionText deve mudar com 3+ pontos', () {
      controller.selectTool('polygon');
      controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
      controller.appendDrawingPoint(const LatLng(-15.7802, -47.9293));
      controller.appendDrawingPoint(const LatLng(-15.7803, -47.9294));
  ...
  group('FIX-DRAW-FLOW-02 — Edge Cases', () {
    ...
    test('🚫 appendDrawingPoint em idle não deve fazer nada', () {
      // Act
      controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
    ...
    test('🚫 appendDrawingPoint sem selectTool não deve adicionar ponto', () {
      // Act
      controller.appendDrawingPoint(const LatLng(-15.7801, -47.9292));
    ...
    test('🔐 selectTool com ferramenta inválida deve resultar em idle', () {
      // Act
    ...
    test('🔐 Múltiplos taps no mesmo ponto devem funcionar', () {
      controller.selectTool('polygon');
  ...
  group('FIX-DRAW-FLOW-02 — Diferentes Ferramentas', () {
    ...
    for (final tool in [
      ...
      test('🛠️ selectTool($tool) deve armar corretamente', () {
