...
/// BUG-008 — Limite de cases publicados: Publicar não trava; salva rascunho
/// (Não gerado) e aponta Relatórios → Marketing.
void main() {
  group('BUG-008 novo_case_limit_regression', () {
    test('submitCaseFromMap no limite chama saveAsDraft e snackbar Relatórios', () {
      final source = File(
        'lib/ui/screens/map/handlers/novo_case_modal_launcher.dart',
      ).readAsStringSync();

      expect(source.contains('static Future<void> submitCaseFromMap'), isTrue);
      expect(source.contains('.saveAsDraft(newCase)'), isTrue);
      expect(source.contains('Salvo como Não gerado em'), isTrue);
      expect(source.contains('Relatórios → Marketing'), isTrue);
      expect(source.contains('PlanoBlockSheet'), isFalse);
    });
