...
### Clima
Contexto climático do mapa. DT-028 encerrado (ADR-043): radar em `clima/`
via `climaRadarEnabledProvider` + `IRadarOverlayController`.
...
| DT-028 — `showRadarProvider` como proxy temporário | Encerrado; ADR-043 (`clima/` + `IRadarOverlayController`) |
