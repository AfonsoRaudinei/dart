-- repair — live pyoejhhkjlrjijiviryq: client_id, geometry e colunas external soil ausentes.
...
begin
  if to_regclass('public.clients') is not null
     and not exists (
       ...
       where conname = 'occurrences_client_id_fkey'
         and conrelid = 'public.occurrences'::regclass
     ) then
    alter table public.occurrences
      add constraint occurrences_client_id_fkey
