...
- IPA 178: **archive OK, export IPA falhou (codesign)** | `build/ios/archive/Runner.xcarchive` | `CFBundleVersion=178` | `CFBundleShortVersionString=1.34.0` | branch `cursor/fix-active-visit-sheet-17d6` @ `93f2e78` | fix toque vértice mid-draw (hit-test map onTap + gota 44dp + tapSketchVertex) | erro: `No signing certificate "iOS Distribution" found` — Xcode → Settings → Accounts → gerar certificado Distribution → `./build_testflight.sh` ou export manual do archive
